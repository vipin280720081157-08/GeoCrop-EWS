...
export default function SystemCheckOverlay() {
  ...
  useEffect(() => {
    if (!visible) return;
    // Automatically collapse after 3 seconds
  ...
  return (
    <div className="fixed top-4 right-4 md:top-6 md:right-6 z-[100] w-[calc(100vw-2rem)] sm:w-80 p-4 rounded-card bg-card border border-borderC shadow-xl fade-in text-textPrimary transition-all duration-200 pointer-events-auto">
      <div className="flex items-start justify-between gap-2 border-b border-borderC pb-2.5 mb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h4 className="font-bold text-sm text-textPrimary leading-tight m-0">GeoCrop System Check</h4>
            <p className="text-[11px] text-textSecondary m-0">Real-time Telemetry &amp; Status</p>
          </div>
        </div>
        <button
          onClick={handleClose}
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        ...
        <Item
          ...
          text={systemStatus.aiModel === "available" ? "Available" : systemStatus.aiModel === "checking" ? "Checking..." : "Unavailable"}
        />
        <Item
          label="GPS"
